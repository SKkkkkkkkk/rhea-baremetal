...
#define SEEHI_RC_PCIE_TEST			1
#define SEEHI_RC_NPU_PCIE_TEST		0
...
void pcie_irq_handler(void)
{
	...
	if (reg & A510_APB_PCIE_RADM_MSG_LTR_INT){
		...
		if(val & A510_APB_PCIE_RADM_MSG_LTR){
			...
			printf("seehi--> %s line: %d req 0x%x byte8 0x%x byte12 0x%x\n", __func__, __LINE__, req, byte8[0], byte12[0]);
		}
	}
...
#error
	...
	if (hdma_int_flag == 1)
	{
		printf("PCIe_RC_hdma start !!!\n");
		hdma_core_off();
		dw_hdma_v0_core_start();
		delay(10);
		hdma_core_ch_status();
		printf("PCIe_RC_hdma end !!!\n");
	}

#if PLD_Z1 & WR_CH0
	dump_regs("hdma:", 0x188b0000, 144);
	dump_regs("llp:",  0x18300000, 64);
	dump_regs("rc apram:",  0x100000, 512);
	dump_regs("ep apram:",  0xd400000000, 512);
#elif PLD_Z2 & WR_CH0	
	dump_regs("hdma:", 0x1c8b0000, 144);
	dump_regs("llp:",  0x1c300000, 64);
	dump_regs("rc apram:",  0x100000, 512);
	dump_regs("ep apram:",  0xf400000000, 32);
#endif

#if PLD_Z1 & RD_CH0
	dump_regs("hdma:", 0x188b0100, 144);
	dump_regs("llp:",  0x18302000, 64);
	dump_regs("rc apram:",  0x100000, 512);
	dump_regs("ep apram:",  0xd400000000, 512);
#elif PLD_Z2 & RD_CH0	
	dump_regs("hdma:", 0x1c8b0100, 144);
	dump_regs("llp:",  0x1c302000, 64);
	dump_regs("rc apram:",  0x100000, 512);
	dump_regs("ep apram:",  0xf400000000, 32);
#endif

#if PLD_Z1 & WR_CH1
	dump_regs("hdma:", 0x188b0200, 144);
	dump_regs("llp:",  0x18304000, 64);
	dump_regs("rc apram:",  0x100000, 512);
	dump_regs("ep apram:",  0xd400000000, 512);
#elif PLD_Z2 & WR_CH1	
	dump_regs("hdma:", 0x1c8b0200, 144);
	dump_regs("llp:",  0x1c304000, 64);
	dump_regs("rc apram:",  0x100000, 512);
	dump_regs("ep apram:",  0xf400000000, 32);
#endif

#if PLD_Z1 & RD_CH1
	dump_regs("hdma:", 0x188b0300, 144);
	dump_regs("llp:",  0x18306000, 64);
	dump_regs("rc apram:",  0x100000, 512);
	dump_regs("ep apram:",  0xd400000000, 512);
#elif PLD_Z2 & RD_CH1	
	dump_regs("hdma:", 0x1c8b0300, 144);
	dump_regs("llp:",  0x1c306000, 64);
	dump_regs("rc apram:",  0x100000, 512);
	dump_regs("ep apram:",  0xf400000000, 32);
#endif
