...
#define BOOT_USING_PCIE_C2C_BAR2_CPU_ADDRESS 0x10300000000   //c2c cfg
// #define BOOT_USING_PCIE_C2C_BAR3_CPU_ADDRESS 0x00400000000   //ap sram
#define BOOT_USING_PCIE_C2C_BAR3_CPU_ADDRESS 0x10300600000   //c2c ram
// #define BOOT_USING_PCIE_C2C_BAR4_CPU_ADDRESS 0x00440000000   //ap tcm
#define BOOT_USING_PCIE_C2C_BAR4_CPU_ADDRESS 0x00400100000   //ap ram
															 //
		...
		if(lanes == 8){
			...
			if((val & A510_APB_PCIE_MSG_VDM_VLD_MASK) >> 1 == 1){
				...
				*byte12 = pcie_readl_apb(pcie, A510_APB_PCIE_MSG_VDM_REG1);
				...
				temp0 = *byte8;
				temp1 = *byte12;
				printf("seehi--> %s line: %d x8 no_fifo | byte8 0x%x byte12 0x%x\n", __func__, __LINE__, temp0, temp1);
			...
			if((val & A510_APB_PCIE_MSG_VDM_VLD_MASK) >> 1 == 1){
				...
				*byte12 = pcie_readl_apb(pcie, A510_APB_PCIE_MSG_VDM_REG1);
				...
				temp0 = *byte8;
				temp1 = *byte12;
				printf("seehi--> %s line: %d x16 no_fifo | byte8 0x%x byte12 0x%x\n", __func__, __LINE__, temp0, temp1);
...
#error
	...
	IRQ_SetHandler(HDMA_INT0, dw_hdma_v0_core_handle_int);
	GIC_SetPriority(HDMA_INT0, 0 << 3);
	GIC_EnableIRQ(HDMA_INT0);
	
	IRQ_SetHandler(HDMA_INT1, dw_hdma_v0_core_handle_int);
	GIC_SetPriority(HDMA_INT1, 0 << 3);
	GIC_EnableIRQ(HDMA_INT1);
	
	IRQ_SetHandler(HDMA_INT2, dw_hdma_v0_core_handle_int);
	GIC_SetPriority(HDMA_INT2, 0 << 3);
	GIC_EnableIRQ(HDMA_INT2);
	
	IRQ_SetHandler(HDMA_INT3, dw_hdma_v0_core_handle_int);
	GIC_SetPriority(HDMA_INT3, 0 << 3);
	GIC_EnableIRQ(HDMA_INT3);
	
#if WR_CH0 | WR_CH1
	hdma_write_data(0x100000,0x100200);
#endif
	
	BSP_PCIE_EP_VDM(pcie, cnt++, 0x53, 0x48);
	printf("PCIe_EP_hdma register !!!\n");
