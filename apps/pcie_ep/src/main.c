...
void LPI_8193_Handler_03_x16(void)
{
	struct HAL_PCIE_DEV *dev = &g_pcieDev_03_x16;
	uint64_t engine_base = dev->engineBase;
...
static void c2c_engine_mbi_tx(struct HAL_PCIE_DEV *dev, int id, int local)
{
	uint64_t mbi_tx_base = dev->mbitxBase;
	uint32_t val;
...
static void c2c_engine_mbi_tx_dump(struct HAL_PCIE_DEV *dev)
{
	uint64_t mbi_tx_base = dev->mbitxBase;
...
static void dsc_table_init(struct HAL_PCIE_DEV *dev, uint32_t tcm_addr_h, uint32_t tcm_addr_l, uint32_t size)
{
...
static void tcm_data_init(struct HAL_PCIE_DEV *dev, uint32_t tcm_addr_h, uint32_t tcm_addr_l, uint32_t size)
{
	uint32_t i;
	uint64_t tcm_addr = (uint64_t)tcm_addr_h << 32 | tcm_addr_l;
...
static void prefetch_dscpt_eng(struct HAL_PCIE_DEV *dev, uint32_t addr_h, uint32_t addr_l)
{
	uint64_t engine_base = dev->engineBase;
...
static void c2c_engine_init(struct HAL_PCIE_DEV *dev)
{
	uint64_t engine_base = dev->engineBase;
...
static void c2c_engine_dump(struct HAL_PCIE_DEV *dev)
{
	uint64_t engine_base = dev->engineBase;
...
static void c2c_engine_dump_addr(struct HAL_PCIE_DEV *dev, uint32_t ddr)
{
	uint32_t i;
...
int main()
{
	...
	if(tile == TILE_03 && control == HAL_X16){
		rhea_pcie_ep_init(&dev, TILE_03, HAL_X16, C2C_EP);  //c2c ep
		// rhea_pcie_ep_init(&dev, TILE_03, HAL_X16, X86_EP);  //x86 ep
		if(ret != 0)
	...
	if(tile == TILE_03 && control == HAL_X8){
		rhea_pcie_ep_init(&dev, TILE_03, HAL_X8, C2C_EP);
		if(ret != 0)
			printf("rhea_pcie_ep_init error\n");
	...
	if(tile == TILE_03 && control == HAL_X16toX8){
		rhea_pcie_ep_init(&dev, TILE_03, HAL_X16toX8, C2C_EP);
		if(ret != 0)
			printf("rhea_pcie_ep_init error\n");
		memcpy(&g_pcieDev_03_x16, &dev, sizeof(struct HAL_PCIE_DEV));
	...
	if(tile == TILE_73 && control == HAL_X16){
		rhea_pcie_ep_init(&dev, TILE_73, HAL_X16, C2C_EP);
		if(ret != 0)
			printf("rhea_pcie_ep_init error\n");
...
#endif
	IRQ_SetHandler(8193, LPI_8193_Handler_03_x16);

	c2c_engine_mbi_tx(dev, 0x21 << 12, 0x21 << 12);
	c2c_engine_init(dev);

	dsc_table_init(dev, 0x00, 0x40051000, 2);  // desc addr
	tcm_data_init(dev, 0x00, 0x40060000, 0xf);  // src addr
	tcm_data_init(dev, 0x00, 0x40061000, 0xf);  // src addr
	prefetch_dscpt_eng(dev, 0x04, 0x40051000);  // start
#endif
...
#if SEEHI_PLD_PCIE_TEST && SEEHI_C2C_ENGINE_TEST
		// c2c_engine_mbi_tx_dump(dev);
		// c2c_engine_dump(dev);
		// c2c_engine_dump_addr(dev, 0x40000000);
		// c2c_engine_dump_addr(dev, 0x40051000);
		// c2c_engine_dump_addr(dev, 0x40060000);
		// c2c_engine_dump_addr(dev, 0x40070000);
#endif
