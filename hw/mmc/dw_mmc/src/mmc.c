...
static int mmc_device_state(void)
{
	...
	do {
		...
		if (retries == 0) {
			printf("CMD13 failed after %d retries\n",
...
static int mmc_sd_switch(unsigned int bus_width)
{
	...
	do {
		...
		if ((ret != 0) && (retries == 0)) {
			printf("ACMD51 failed after %d retries (ret=%d)\n",
...
static int mmc_set_ios(unsigned int clk, unsigned int bus_width)
{
	...
	if (mmc_dev_info->mmc_dev_type != MMC_IS_EMMC) {
		if (width == MMC_BUS_WIDTH_8) {
			printf("Wrong bus config for SD-card, force to 4\n");
		...
		printf("Wrong MMC type or spec version\n");
	}
...
static int sd_send_op_cond(void)
{
	...
	printf("ACMD41 failed after %d retries\n", SEND_OP_COND_MAX_RETRIES);
...
static int mmc_send_op_cond(void)
{
	...
	printf("CMD1 failed after %d retries\n", SEND_OP_COND_MAX_RETRIES);
...
int mmc_part_switch_current_boot(void)
{
	...
	if ((current_boot_part != 1U) && (current_boot_part != 2U)) {
		printf("Got unexpected value for active boot partition, %u\n", current_boot_part);
		return -EIO;
	}
...
int mmc_part_switch_user(void)
{
	...
	if (ret < 0) {
		printf("Failed to switch to user partition, %d\n", ret);
	}
