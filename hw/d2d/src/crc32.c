...
static inline uint32_t htonl(uint32_t hostlong) {
    uint32_t netlong = 0;
    uint8_t *host_byte = (uint8_t *)&hostlong;
    uint8_t *net_byte = (uint8_t *)&netlong;
 
    net_byte[0] = host_byte[3];
    net_byte[1] = host_byte[2];
    net_byte[2] = host_byte[1];
    net_byte[3] = host_byte[0];
    return netlong;
}
