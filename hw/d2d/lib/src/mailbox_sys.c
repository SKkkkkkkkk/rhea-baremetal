 ...
 * return value:
 * positive integer: the size of data return back from the slave
...
int mailbox_sys_send(uint8_t *in_buff, uint8_t in_len, uint8_t *out_buff, uint8_t out_len, uint32_t timeout_ms)
{
	...
	ret = mailbox_send((uint8_t *)in_buff, in_len, MAILBOX_MSG_NEED_REPLY, timeout_ms);

	if (CLCI_SUCCESS != ret)
		return ret;

	ret = mailbox_rev(out_buff, out_len, &reply, timeout_ms);

	return ret;
}
/*
 * As the master, send specific length data to the slave via the mailbox
 * the deference between this API and mailbox_sys_send is that there is no replay from the receiver
 * parameters:
 * in_buff: points to the data to be send
 * in_len: the length of the data to be send
 * timeout_ms: the maximum time given to this function to executing, if the actual time is too long
 *            CLCI_E_TIMEOUT will return.
 * return value:
 * CLCI_SUCCESS: send successfully
 * CLCI_E_TIMEOUT: timeout
 * CLCI_E_DEVICE: the mailbox device is error
 */

int mailbox_sys_send_noack(uint8_t *in_buff, uint8_t in_len, uint32_t timeout_ms)
{
	int ret;

	ret = mailbox_send((uint8_t *)in_buff, in_len, MAILBOX_MSG_N0_REPLY, timeout_ms);
